...
public class QuestionTargetDaoImpl implements QuestionTargetDao {
    ...
    public QuestionTargetData getById(Integer id) {
        String sql = "SELECT `id`,`pid`,`title`,`status`,`sort`,`remark` FROM question_target WHERE `id`=:id";
        ...
        if (targetData != null) {
            String childSql = "SELECT `id`,`pid`,`title`,`status`,`sort`,`remark` FROM question_target WHERE `pid`=:id ORDER BY `sort` ASC";
    ...
    public Integer insert(Map<String, Object> param) {
        ...
        String sql = "INSERT INTO question_target(`pid`,`title`,`sort`,`status`,`remarks`) VALUES(:pid, :title, :sort, 1, :remarks)";
        try {
